...
def build_opt1_zip_and_excel(records):
    try:
        ...
        zip_buffer.seek(0)
        res = zip_buffer.getvalue()
        return res if len(res) > 500 else None
    except Exception as e:
        print(f"Lỗi build zip opt1: {e}")
        return None

# Dọn dẹp rác bộ nhớ đệm Python ngay sau khi tạo xong
        gc.collect()
        return res if len(res) > 500 else None
    except Exception as e:
        print(f"Lỗi build zip opt1: {e}")
...
if option == "1. Quét Mã Vạch & Tách PDF (Zip + Excel)":
    ...
    if uploaded_pdfs:
        ...
        if st.button("🚀 BẮT ĐẦU XỬ LÝ", key="btn_start_opt1"):
           ...
           # Tự động dọn dẹp các biến rác trong bộ nhớ trước khi xử lý mới
            gc.collect()
            st.session_state.zip_data = None
            st.session_state.opt1_records = []
            st.session_state.processed_option = None
            ...
            def process_opt1_page(args):
                ...
                try:
                    ...
                    page_images = convert_from_bytes(pdf_bytes_inner, dpi=200, first_page=page_num, last_page=page_num, thread_count=1)
            ...
            try:
                ...
                zip_bytes = build_opt1_zip_and_excel(all_records)
    ...
    if st.session_state.processed_option == "opt1" and st.session_state.opt1_records:
        ...
        if st.button("🔄 CẬP NHẬT DỮ LIỆU ĐÃ SỬA", key="btn_update_opt1"):
            ...
            if changes_to_apply:
                ...
                if new_zip:
                    ...
                    st.session_state.zip_data = new_zip
        ...
        if st.session_state.zip_data:
            ...
            st.download_button("📥 TẢI FILE ZIP KẾT QUẢ", data=st.session_state.zip_data, file_name="KET_QUA_POD.zip", mime="application/zip", type="primary", key="dl_opt1",on_click="ignore")
 # ==========================================================================================

# OPTION 2: BSH_OCR & TÁCH PDF TỰ ĐỘNG (GOM CHUNG 1 THƯ MỤC + 1 EXCEL TỔNG HỢP)
...
# ==========================================================================================
    ...
    if uploaded_files:
        ...
        if st.button("🚀 BẮT ĐẦU XỬ LÝ", key="btn_start_opt2"):
            ...
            # Tự động dọn dẹp bộ nhớ RAM trước khi xử lý mới
            ...
            try:
                ...
                for file_idx, uploaded_file in enumerate(uploaded_files):
                    ...
                    for p_idx, (page_num, final_code, doc_type, status_str, preview_img_bytes, pdf_bytes_page) in enumerate(page_results):
                        ...
                        # Tự động gán mã chữ cái A, B, C... nếu không đọc được mã
                ...
                st.session_state.zip_data = build_opt2_zip_and_excel(all_records)
            ...
            if changes_to_apply:
                ...
                for rec in st.session_state.opt2_records:
                    ...
                    if rec["stt"] in changes_to_apply:
                        ...
                        # Tự động cập nhật lại loại chứng từ dựa theo tiền tố mã mới nhập
                ...
                # Tạo lại dữ liệu ZIP và Excel mới với các giá trị đã cập nhật

                st.session_state.zip_data = build_opt2_zip_and_excel(st.session_state.opt2_records)
